...
│  ── MainMenuPage, MapPage, DeveloperPage                       │
├───────────────────────────────────────────────────────────────┤
│  PageModels (MVVM) — stan widoku, komendy                      │
│  ── MainMenuPageModel, MapPageModel, DeveloperPageModel        │
├───────────────────────────────────────────────────────────────┤
...
- **`WagonColor`** (enum) — paleta gracza wybierana w menu głównym: `Czarny, Czerwony, Niebieski, Zielony, Żółty` (2.5).
...
- **`GameStateService`** (singleton) — w pamięci: oznaczone miasta, stany tras, `WagonColor`,
  ...
  właściwość `HasActivePlan` (czy w sesji rozpoczęto rozgrywkę — steruje opcją
...
- **`MapPageModel`** — projekcja `GameStateService`; `ToggleCityCommand`, `CycleRouteCommand`;
  ...
  `SelectSearchResultCommand` (zoom-to-city + oznaczenie, 2.7); przyciski przybliżania/oddalania mapy (2.1).
...
- **`MapPage`** — pełnoekranowa plansza z gestami **pinch-to-zoom** i **pan** oraz przyciskami
...
- **Model nawigacji:** `MainMenuPage` jest ekranem głównym (root): wybór koloru i rozpoczęcie/
  kontynuacja rozgrywki (→ `MapPage`) albo wejście w tryb deweloperski. Akcje destrukcyjne (nowa
  rozgrywka) i wybór koloru dostępne są wyłącznie z menu głównego — nie bezpośrednio z mapy (2.5).
...
├── PageModels/   MainMenuPageModel (+ ColorChoice), DeveloperPageModel  (MapPage → code-behind)
├── Pages/        MainMenuPage, MapPage, DeveloperPage  (.xaml / .cs)
